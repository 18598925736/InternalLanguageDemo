...
class SkinFactory : Factory2 {
    private var mDelegate //预定义一个委托类，它负责按照系统的原有逻辑来创建view
            : AppCompatDelegate? = null
    private val listCacheSkinView: MutableList<SkinView> =
        ArrayList() //我自定义的list，缓存所有可以换肤的View对象
    ...
    override fun onCreateView(
        ...
        var view: View? = mDelegate!!.createView(
            parent,
            name,
            context,
            attrs
    ...
    private val mConstructorArgs = arrayOfNulls<Any>(2) //View的构造函数的2个"实"参对象
    ...
    private fun createViewByPrefix(
        ...
        if (constructor == null) {
            try {
                ...
                constructor =
                    clazz.getConstructor(*mConstructorSignature) //拿到 构造方法，
            } catch (e: Exception) {
                e.printStackTrace()
                return null
            }
    ...
    companion object {
        ...
        private val sConstructorMap =
            HashMap<String, Constructor<out View>?>() //用映射，将View的反射构造函数都存起来
        val prefixs =
            arrayOf( //安卓里面控件的包名，就这么3种,这个变量是为了下面代码里，反射创建类的class而预备的
                "android.widget.",
                "android.view.",
                "android.webkit."
            )
